...
사용자 입력 데이터를 분석하여 지능형 페르소나를 생성하고 저장하는 상세 흐름입니다. 이 과정은 `v2-shared`의 DBManager를 참조하며, 작업 시간이 소요되는 분석 로직은 비동기로 처리됩니다.
...
graph TD
    ...
    PromptMD <--> Mongo_P[("S3 Bucket (prompts col)")]
    PersonaDesc -- Update --> Mongo_U[("MongoDB (users col)")]
```
...
- **Prompt Management (LLMOps)**: BackgroundTask가 시작되면 외부 스토리지(S3)로부터 최신 **Prompt Markdown** 파일을 호출합니다.
...
**2. AI 분석 재시도 및 실패 처리**
- **재시도 정책**: AI 분석 호출 실패 시 **최대 3회 고정 재시도**를 수행합니다.
- **최종 실패 시 처리**: 
...
**3. 병렬 처리 전략**
- 추가로 그래프의 복잡성이 증가할 경우 병렬 처리를 고려할 수 있습니다.