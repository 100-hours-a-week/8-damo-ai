    ...
    try:
        ...
        config = {
            ...
            "configurable": {
                ...
                "thread_id": str(dining_id),  # 체크포인터 재개용
            },
        ...
        if checkpointer is not None:
            ...
            if existing is not None:
                logger.info("[%s] 체크포인트 복원 → 이어서 실행: dining_id=%s", log_type.upper(), dining_id)
                graph_input = None  # ainvoke(None): 체크포인트 상태를 그대로 사용
                # ※ ainvoke(initial_state)를 넘기면 체크포인트 필드(filtered_restaurant 등)를
                #   초기값으로 덮어써 처음부터 재실행하게 된다 — 반드시 None을 사용할 것
