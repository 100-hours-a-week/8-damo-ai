...
def _format_dining_info(dining_data: Dict[str, Any]) -> str:
    ...
    if dining_data.get("budget"):
        lines.append(f"전체 팀 예산: {dining_data['budget']:,}원")
...
def _format_restaurant_info(
    ...
    # 리뷰 키워드 (상위 5개)
    keywords = (
        restaurant.get("review_keywords")
        or restaurant.get("restaurantReviewKeywords")
        or []
    )
    if keywords:
        kw_list = [
            k.get("keyword", k) if isinstance(k, dict) else str(k) for k in keywords[:5]
        ]
    ...
    allergy_info = (
        f"식당 메뉴 전체: {all_menu_titles}" if all_menu_titles else "메뉴 정보 없음"
    )
...
@observe(name="analyst_speak", as_type="generation")
async def _analyst_speak(
    ...
    obs: Dict[str, Any] = {"model": settings.OPENAI_MODEL}
    if lf_prompt:
        obs["prompt"] = lf_prompt
...
async def _persona_react(
    ...
    obs: Dict[str, Any] = {"name": settings.LOCAL_MODEL or "persona_speak", "model": settings.LOCAL_MODEL}
    if lf_prompt:
        obs["prompt"] = lf_prompt
    if hasattr(response, "usage_metadata") and response.usage_metadata:
        obs["usage_details"] = {
...
async def _persona_vote(
    ...
    response = await llm.ainvoke([SystemMessage(content=system_prompt)] + messages)
    obs: Dict[str, Any] = {"name": settings.LOCAL_MODEL or "persona_vote", "model": settings.LOCAL_MODEL}
    if lf_prompt:
        obs["prompt"] = lf_prompt
    if hasattr(response, "usage_metadata") and response.usage_metadata:
        obs["usage_details"] = {
...
async def restaurant_dialogue(state: AgentDialogueState, config: RunnableConfig) -> dict:
    ...
    # 2. 분석가 발언 생성 (API LLM)
    try:
        analyst_speech = await _analyst_speak(
            restaurant_info, dining_info, allergy_info
        )
    except Exception:
        logger.warning("[Node3] 분석가 발언 생성 실패", exc_info=True)
        return {
            "is_error": True,
            "error_message": f"분석가 발언 생성 실패: {place_name}",
        }
    ...
    dialogue_history.append(analyst_entry)
    if on_speak:
        result = on_speak(analyst_entry)
    ...
    for user_id, system_prompt in persona_prompts.items():
        ...
        try:
            content = await _persona_react(
                system_prompt, analyst_speech, previous_reactions
            )
        except Exception:
            logger.warning(
                "[Node3] 페르소나 반응 실패: user_id=%s", user_id, exc_info=True
            )
            content = "(반응 실패)"
        ...
        dialogue_history.append(reaction_entry)
        if on_speak:
            result = on_speak(reaction_entry)
            if inspect.isawaitable(result):
                await result
