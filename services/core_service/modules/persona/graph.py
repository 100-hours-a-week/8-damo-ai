...
from shared.storage.bucket_manager import BucketManager
...
async def fetch_prompt_node(state: PersonaState, config: RunnableConfig):
    ...
    if not prompt:
        logger.error("❌ Persona prompt configuration not found in DB")
        return {"error_message": "Prompt config not found", "retry_count": state.get("retry_count", 0) + 1}
        
    version = prompt.get('shouldUseVersion', 'v1')
    prompt_file_name = f"persona_{version}.md"
    
    success = bucket_manager.download_file(prompt_file_name, f"/tmp/{prompt_file_name}")
    if not success:
        logger.error(f"❌ Failed to download prompt from bucket: persona/{prompt_file_name}")
        return {"error_message": "Prompt download failed", "retry_count": state.get("retry_count", 0) + 1}
    
    try:
        with open(f"/tmp/{prompt_file_name}", "r") as f:
            raw_content = f.read()
    except Exception as e:
        logger.error(f"❌ Failed to read downloaded prompt file: {e}")
        return {"error_message": f"Prompt read failed: {e}", "retry_count": state.get("retry_count", 0) + 1}
    finally:
        if os.path.exists(f"/tmp/{prompt_file_name}"):
